...
freq1_nondim = freq1 / kR
freq2_nondim = freq2 / kR
freq3_nondim = freq3 / kR

Tfiber = []
nfirings1 = round(Int, tfinal/3 * freq1_nondim)    # switching between frequencies
nfirings2 = round(Int, tfinal/3 * freq2_nondim)
nfirings3 = round(Int, tfinal/3 * freq3_nondim)
for fiberidx in firingfibers
    append!(Tfiber, [vcat([0.6*kR + (k-1) * 1/freq1_nondim for k in 1:nfirings1], [0.6*kR + tfinal/3 + (k-1) * 1/freq2_nondim for k in 1:nfirings2], [0.6*kR + 2*tfinal/3 + (k-1) * 1/freq3_nondim for k in 2:nfirings3+1])])          # switching between frequencies
end
...
# and to compute the exponential integral E₁(σt) = 1/(2πi) ∫ᵧ U(s) exp(st) ds
# with U(s) = ln(1+s/σ)/s
M = m
...
for tidx1 in 1:tfineness_factor
    ...
    k1 = fvec(t, μarray2)
    k2 = fvec(t + Δt/2, μarray2 .+ Δt_finer/2 .* k1)
    k3 = fvec(t + Δt/2, μarray2 .+ Δt_finer/2 .* k2)
    k4 = fvec(t + Δt, μarray2 .+ Δt_finer .* k3)
...
### Instead with increased fineness
# now using μarray[2, :] for the more refined B(Δt) asymptotic expression
...
for tidx2 in 1:tfineness_factor
    ...
    k1 = fvec(t, μarray3)
    k2 = fvec(t + Δt/2, μarray3 .+ Δt_finer/2 .* k1)
    k3 = fvec(t + Δt/2, μarray3 .+ Δt_finer/2 .* k2)
    k4 = fvec(t + Δt, μarray3 .+ Δt_finer .* k3)
...
for tidx in 3:ntpoints - 1
    ...
    # Rk4 weights
    k1 = fvec(t, μarray[tidx, :])
    k2 = fvec(t + Δt/2, μarray[tidx, :] .+ Δt_finer/2 .* k1)
    k3 = fvec(t + Δt/2, μarray[tidx, :] .+ Δt_finer/2 .* k2)
    k4 = fvec(t + Δt, μarray[tidx, :] .+ Δt_finer .* k3)
...
writedlm("./datasets/tarray6s.txt", tarray)
writedlm("./datasets/mu_array_$(freq1)Hz_$(freq2)Hz_$(freq3)Hz.txt", μarray)
writedlm("./datasets/B_array_$(freq1)Hz_$(freq2)Hz_$(freq3)Hz.txt", Barray)
