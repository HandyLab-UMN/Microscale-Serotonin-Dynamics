...
# first time step 0 → Δt (with increased fineness)
...
writedlm("./datasets/tarray60s.txt", tarray)
writedlm("./datasets/mu_array_freq$(freq)_$(n_on_fiber)line_$(modelType).txt", μarray)
writedlm("./datasets/B_array_freq$(freq)_$(n_on_fiber)line_$(modelType).txt", Barray)
